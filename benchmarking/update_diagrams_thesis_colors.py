...
Using ONLY thesis colors and with proper layering (arrows in background)
"""
...
def create_comprehensive_data_flow():
    ...
    ax.text(8, 11.5, 'MEGA Architectural Mentor - Complete Data Flow', 
            fontsize=20, weight='bold', ha='center', color=THESIS_COLORS['primary_dark'])
    
    # FIRST: Draw all arrows (background layer)
    arrow_props = dict(arrowstyle='->', lw=2.5, color=THESIS_COLORS['primary_purple'], alpha=0.6, zorder=1)
    component_arrow_props = dict(arrowstyle='->', lw=1.5, color=THESIS_COLORS['primary_rose'], alpha=0.5, zorder=1)
    
    # Main flow arrows
    ax.annotate('', xy=(5, 9.75), xytext=(4, 9.75), arrowprops=arrow_props)
    ax.annotate('', xy=(9.5, 9.75), xytext=(8.5, 9.75), arrowprops=arrow_props)
    ax.annotate('', xy=(13.5, 9.75), xytext=(13, 9.75), arrowprops=arrow_props)
    
    # Vertical arrows to components
    ax.annotate('', xy=(2, 7.5), xytext=(2.25, 8.5), arrowprops=component_arrow_props)
    ax.annotate('', xy=(5, 7.5), xytext=(6.75, 8.5), arrowprops=component_arrow_props)
    ax.annotate('', xy=(8, 7.5), xytext=(8, 8.5), arrowprops=component_arrow_props)
    
    # From components to storage
    ax.annotate('', xy=(3, 4.7), xytext=(2, 5), arrowprops=component_arrow_props)
    ax.annotate('', xy=(6, 4.7), xytext=(5, 5), arrowprops=component_arrow_props)
    ax.annotate('', xy=(9, 4.7), xytext=(8, 5), arrowprops=component_arrow_props)
    ax.annotate('', xy=(12, 4.7), xytext=(11, 5), arrowprops=component_arrow_props)
    
    # Export arrow
    ax.annotate('', xy=(14.25, 3), xytext=(14.5, 8.5),
                arrowprops=dict(arrowstyle='->', lw=2, color=THESIS_COLORS['accent_coral'], alpha=0.6, zorder=1))
    
    # SECOND: Draw all boxes (foreground layer)
    ...
    components = [
        ('Linkography\nAnalysis', 2, 6, THESIS_COLORS['primary_rose']),
        ('Graph ML\nProcessing', 5, 6, THESIS_COLORS['accent_magenta']),
        ('Proficiency\nClassifier', 8, 6, THESIS_COLORS['primary_violet']),
        ('Statistical\nAnalysis', 11, 6, THESIS_COLORS['primary_purple']),
        ('Report\nGenerator', 14, 6, THESIS_COLORS['accent_coral'])
    ]
    ...
    export_box = FancyBboxPatch((13, 1.5), 2.5, 1.5,
                                boxstyle="round,pad=0.1",
                                facecolor=THESIS_COLORS['neutral_orange'],
                                edgecolor=THESIS_COLORS['primary_dark'],
                                linewidth=2, alpha=0.9, zorder=2)
    ...
    # THIRD: Add all text (topmost layer)
    # Main stages text
    ...
    for name, x, y in storage_boxes:
        ax.text(x, y, name, fontsize=9,
                ha='center', va='center', color=THESIS_COLORS['primary_dark'],
                multialignment='center', zorder=3)
    ...
    ax.text(6, 1.6, 'KEY FEATURES', fontsize=11, weight='bold',
            ha='center', va='center', color=THESIS_COLORS['primary_dark'], zorder=3)
    ax.text(2.5, 1, '• Real cognitive assessment', fontsize=9,
            ha='left', va='center', color=THESIS_COLORS['primary_dark'], zorder=3)
...
def create_metric_calculation_flow():
    ...
    ax.text(7, 9.5, 'Cognitive Metric Calculation Flow', 
            fontsize=18, weight='bold', ha='center', color=THESIS_COLORS['primary_dark'])
    
    # FIRST: Draw all arrows (background)
    arrow_props = dict(arrowstyle='->', lw=1.5, color=THESIS_COLORS['primary_purple'], alpha=0.5, zorder=1)
    ...
    metrics = [
        ('Prevention Rate\nCalculation', 3, 3.5, THESIS_COLORS['primary_purple']),
        ('Deep Thinking\nAssessment', 7, 3.5, THESIS_COLORS['primary_purple']),
        ('Improvement\nScore', 11, 3.5, THESIS_COLORS['primary_purple'])
    ]
    ...
    ax.text(7, 0.5, 'Prevention Rate = 1 - (Direct Answer Keywords / Total Keywords) × Response Quality Factor',
            fontsize=9, ha='center', va='center', color=THESIS_COLORS['primary_dark'], zorder=3)
...
def create_linkography_analysis_flow():
    ...
    # FIRST: Draw all arrows (background)
    arrow_props = dict(arrowstyle='->', lw=2, color=THESIS_COLORS['primary_purple'], alpha=0.6, zorder=1)
    component_arrows = dict(arrowstyle='->', lw=1.5, color=THESIS_COLORS['primary_rose'], alpha=0.5, zorder=1)
    
    # Horizontal flow
    ax.annotate('', xy=(4.5, 8.25), xytext=(3.5, 8.25), arrowprops=arrow_props)
    ax.annotate('', xy=(8.5, 8.25), xytext=(7.5, 8.25), arrowprops=arrow_props)
    ax.annotate('', xy=(11.5, 8.25), xytext=(11, 8.25), arrowprops=arrow_props)
    
    # To components
    ax.annotate('', xy=(2, 5.7), xytext=(6, 7.5), arrowprops=component_arrows)
    ax.annotate('', xy=(5, 5.7), xytext=(6, 7.5), arrowprops=component_arrows)
    ax.annotate('', xy=(8, 5.7), xytext=(10, 7.5), arrowprops=component_arrows)
    ax.annotate('', xy=(11, 5.7), xytext=(12.5, 7.5), arrowprops=component_arrows)
    
    # To outputs
    ax.annotate('', xy=(3, 3.2), xytext=(2, 4.5), arrowprops=component_arrows)
    ax.annotate('', xy=(7, 3.2), xytext=(8, 4.5), arrowprops=component_arrows)
    ax.annotate('', xy=(11, 3.2), xytext=(11, 4.5), arrowprops=component_arrows)
    ...
    # Analysis Components
    components = [
        ('Pattern\nRecognition', 2, 5, THESIS_COLORS['primary_rose']),
        ('Critical Moves\nIdentification', 5, 5, THESIS_COLORS['accent_magenta']),
        ('Phase\nClassification', 8, 5, THESIS_COLORS['primary_violet']),
        ('Cognitive\nMapping', 11, 5, THESIS_COLORS['primary_purple'])
    ]
...
def create_graph_ml_pipeline():
    ...
    # FIRST: Draw all arrows (background)
    arrow_props = dict(arrowstyle='->', lw=2, color=THESIS_COLORS['primary_purple'], alpha=0.6, zorder=1)
    feature_arrows = dict(arrowstyle='->', lw=1.5, color=THESIS_COLORS['primary_rose'], alpha=0.5, zorder=1)
    
    # Main flow
    ax.annotate('', xy=(3.5, 8.25), xytext=(3, 8.25), arrowprops=arrow_props)
    ax.annotate('', xy=(6.5, 8.25), xytext=(6, 8.25), arrowprops=arrow_props)
    ax.annotate('', xy=(9.5, 8.25), xytext=(9, 8.25), arrowprops=arrow_props)
    ax.annotate('', xy=(12.5, 8.25), xytext=(12, 8.25), arrowprops=arrow_props)
    
    # To features/edges
    ax.annotate('', xy=(4.75, 6.3), xytext=(4.75, 7.5), arrowprops=feature_arrows)
    ax.annotate('', xy=(7.75, 4.8), xytext=(7.75, 7.5), arrowprops=feature_arrows)
    
    # To outputs
    ax.annotate('', xy=(7, 1.5), xytext=(10.75, 7.5), arrowprops=feature_arrows)
    ...
    pred_box = FancyBboxPatch((12.5, 7.5), 1.5, 1.5,
                              boxstyle="round,pad=0.1",
                              facecolor=THESIS_COLORS['accent_magenta'],
                              edgecolor=THESIS_COLORS['primary_dark'],
                              linewidth=2, zorder=2)
    ...
    # Features and edges text
    for i, feature in enumerate(node_features):
        ax.text(1.5 + i*2.5, 5.5, feature, fontsize=8,
                ha='center', va='center', color=THESIS_COLORS['primary_dark'], zorder=3)
    
    for i, edge_type in enumerate(edge_types):
        ax.text(3.5 + i*3.5, 4, edge_type, fontsize=9,
                ha='center', va='center', color=THESIS_COLORS['primary_dark'], zorder=3)
...
def create_dashboard_architecture():
    ...
    # Data Sources
    data_sources = [
        ('Master Session\nMetrics CSV', 2, 3.5, THESIS_COLORS['primary_purple']),
        ('Aggregate\nMetrics CSV', 5, 3.5, THESIS_COLORS['primary_purple']),
        ('Benchmark\nReport JSON', 8, 3.5, THESIS_COLORS['primary_purple']),
        ('Evaluation\nReports', 11, 3.5, THESIS_COLORS['primary_purple'])
    ]
    ...
    for name, x, y, _ in data_sources:
        ax.text(x, y, name, fontsize=9, weight='bold',
                ha='center', va='center', color='white',
                multialignment='center', zorder=3)
    ...
    ax.text(7, 0.5, 'Real-time Updates | Interactive Visualizations | Professional Export | No Hardcoded Values',
            fontsize=10, ha='center', va='center', color=THESIS_COLORS['primary_dark'], weight='bold', zorder=3)
...
def create_export_system_flow():
    ...
    # Processing Components
    components = [
        ('Simple Report\nGenerator', 2, 2.5, THESIS_COLORS['primary_purple']),
        ('Insights\nGenerator', 4.5, 2.5, THESIS_COLORS['primary_purple']),
        ('Visualization\nEngine', 7, 2.5, THESIS_COLORS['primary_purple']),
        ('Data\nProcessor', 9.5, 2.5, THESIS_COLORS['primary_purple'])
    ]
    ...
    ax.text(10, 6.3, 'Simple HTML', fontsize=9,
            ha='center', va='center', color=THESIS_COLORS['primary_dark'], zorder=3)
    ax.text(10, 5.9, 'Professional Reports', fontsize=9,
            ha='center', va='center', color=THESIS_COLORS['primary_dark'], zorder=3)
...
if __name__ == "__main__":
    print("Updating all benchmarking diagrams with THESIS COLORS ONLY...")
    ...
    print("\nAll diagrams updated successfully with thesis colors!")
    print("\nDiagrams created:")

...
class CognitiveEnhancementAgent:
    ...
    async def provide_challenge(self, state: ArchMentorState, context_classification: Dict, analysis_result: Dict, routing_decision: Dict) -> Dict[str, Any]:
        ...
        # CREATE CONCISE COGNITIVE ASSESSMENT
    ...
    def _detect_cognitive_offloading_patterns(self, context_classification: Dict, state: ArchMentorState) -> Dict[str, Any]:
        ...
        # PATTERN 1: Premature Answer Seeking
        if (context_classification.get("interaction_type") in ["example_request", "feedback_request"] and
            len(state.messages) < 6):  # Early in conversation
            detection["detected"] = True
            detection["type"] = "premature_answer_seeking"
            detection["confidence"] = 0.8
            detection["indicators"].append("Asking for answers before exploration")
            detection["mitigation_strategy"] = "guided_discovery"
        
        # PATTERN 2: Superficial Confidence
        if (context_classification.get("confidence_level") == "overconfident" and
            context_classification.get("engagement_level") == "low"):
            detection["detected"] = True
            detection["type"] = "superficial_confidence"
            detection["confidence"] = 0.7
            detection["indicators"].append("Overconfident but not engaged")
            detection["mitigation_strategy"] = "assumption_challenge"
        
        # PATTERN 3: Passive Acceptance
        user_messages = [msg['content'] for msg in state.messages if msg.get('role') == 'user']
        if (context_classification.get("engagement_level") == "low" and
            any(len(msg) < 10 for msg in user_messages[-3:] if user_messages)):
            detection["detected"] = True
            detection["type"] = "passive_acceptance"
            detection["confidence"] = 0.7
            detection["indicators"].append("Passive acceptance without engagement")
            detection["mitigation_strategy"] = "engagement_activation"
        
        return detection
    
    async def _generate_cognitive_intervention(self, offloading_detection: Dict, state: ArchMentorState, analysis_result: Dict) -> Dict[str, Any]:
        """Generate specific intervention for cognitive offloading patterns"""
        
        intervention_type = offloading_detection["type"]
        mitigation_strategy = offloading_detection["mitigation_strategy"]
        
        if intervention_type == "premature_answer_seeking":
            response_text = """
**🧠 COGNITIVE PROTECTION ACTIVATED**

I notice you're asking for specific answers early in your design process. This is a common pattern that can limit your creative thinking.

**Instead of giving you the answer, let's explore together:**

1. **What have you already considered about this problem?**
2. **What assumptions are you making?**
3. **What would happen if you approached this from a completely different angle?**

*This approach helps build stronger neural pathways for creative problem-solving.*
"""
        
        elif intervention_type == "superficial_confidence":
            response_text = """
**🧠 COGNITIVE PROTECTION ACTIVATED**

I sense you're confident, but let's make sure that confidence is built on solid thinking.

**Let's challenge your assumptions:**

1. **What evidence supports your current approach?**
2. **What are the potential weaknesses in your reasoning?**
3. **How would you defend this to a skeptical critic?**

*True expertise comes from questioning, not just knowing.*
"""
        
        elif intervention_type == "passive_acceptance":
            response_text = """
**🧠 COGNITIVE PROTECTION ACTIVATED**

I notice you're being quite passive in our interaction. Let's activate your thinking.

**Let's engage your mind:**

1. **What's your gut reaction to this problem?**
2. **What's the most challenging aspect for you?**
3. **How do you feel about taking ownership of this design?**

*Your unique perspective is valuable - let's hear it.*
"""
...
I'm detecting patterns that suggest you might be relying too heavily on external answers. Let's shift to building your independent thinking.

**Let's explore your own reasoning:**
1. **What's your initial instinct about this?**
2. **What questions do you have that we haven't addressed?**
3. **How do you want to approach this differently?**

*The goal is to make you more independent, not more dependent.*
"""
        
        return {
            "agent": self.name,
            "response_text": response_text,
            "response_type": "cognitive_intervention",
...
"""
    ...
    def calculate_scientific_metrics(self, cognitive_state: Dict, state: ArchMentorState, analysis_result: Dict) -> Dict[str, Any]:
        ...
        #3107-ADDED THESIS METRICS
        # Calculate the six thesis metrics
        thesis_metrics = {
            "cognitive_offloading_prevention": self._calculate_cognitive_offloading_prevention(cognitive_state, state),
            "deep_thinking_engagement": engagement_metrics.get("engagement_score", 0.5),
            ...
            "learning_progression": progression_metrics.get("progression_score", 0.5),
            "metacognitive_awareness": reflection_metrics.get("metacognitive_score", 0.5)
        }
        ...
        return {
            ...
            #3107-ADDED THESIS METRICS BELOW LINE
            "thesis_metrics": thesis_metrics,  # Add thesis metrics
    ...
    def validate_thesis_metrics(self, scientific_metrics: Dict) -> Dict[str, Any]:
        ...
        #3107- THESIS METRICS ADDED NAMES CHANGED FROM scientific_metrics to thesis_metrics (if metric_name not in scientific_metrics:)
        # Check thesis_metrics specifically
        thesis_metrics = scientific_metrics.get("thesis_metrics", {})
        
        for metric_name in required_metrics:
            if metric_name not in thesis_metrics:
                validation_result["all_metrics_present"] = False
                validation_result["missing_metrics"].append(metric_name)
            else:
                metric_value = thesis_metrics[metric_name]
                if isinstance(metric_value, (int, float)) and 0 <= metric_value <= 1:
                    validation_result["metric_quality"][metric_name] = "valid"
                else:
                    validation_result["metric_quality"][metric_name] = "invalid_range"
        
        valid_metrics = len([q for q in validation_result["metric_quality"].values() if q == "valid"])
        validation_result["overall_score"] = valid_metrics / len(required_metrics)
        return validation_result
    #3107-ADDED CALCULATE COGNITIVE OFFLOADING PREVENTION;KNOWLEDGE INTEGRATION;SCAFFOLDING EFFECTIVENESS;LEARNING PROGRESSION;METACOGNITIVE AWARENESS
    def _calculate_cognitive_offloading_prevention(self, cognitive_state: Dict, state: ArchMentorState) -> float:
        ...
        # Check for offloading indicators
        user_messages = [msg['content'] for msg in state.messages if msg.get('role') == 'user']
        
        offloading_indicators = 0
        #3107-ADDED BELOW LINE
        positive_engagement_indicators = 0
        total_indicators = 0
        
        # Check for passive acceptance patterns
        passive_patterns = ["okay", "sure", "fine", "whatever", "I guess"]
        for msg in user_messages:
            total_indicators += 1
            if any(pattern in msg.lower() for pattern in passive_patterns):
                offloading_indicators += 1
        
        # Check for direct answer seeking
        answer_seeking_patterns = ["what is the", "tell me the", "give me the", "the answer is"]
        for msg in user_messages:
            total_indicators += 1
            if any(pattern in msg.lower() for pattern in answer_seeking_patterns):
                offloading_indicators += 1
        
        # 3107-ENHANCED: Check for positive engagement patterns
        positive_patterns = [
            "I'm considering", "I'm thinking", "I plan to", "I'll balance",
            "I want to focus", "I'm exploring", "I'll include", "I plan to include",
            "I'm orienting", "I'll use", "I'll create", "I'll design"
        ]
        
        for msg in user_messages:
            total_indicators += 1
            if any(pattern in msg.lower() for pattern in positive_patterns):
                positive_engagement_indicators += 1
        
        #Check for detailed technical responses (good engagement)
        detailed_response_indicators = 0
        for msg in user_messages:
            if len(msg.split()) > 30:  # Detailed response
                detailed_response_indicators += 1
        


        if total_indicators == 0:
            return 1.0  # No offloading detected
        
        # 3107-Calculate prevention score with positive engagement bonus
        base_prevention = 1.0 - (offloading_indicators / total_indicators)
        positive_bonus = min(positive_engagement_indicators / total_indicators, 0.3)
        detailed_bonus = min(detailed_response_indicators / total_indicators, 0.2)
        
        prevention_score = base_prevention + positive_bonus + detailed_bonus
    ...
    def _calculate_scaffolding_effectiveness(self, cognitive_state: Dict, analysis_result: Dict) -> float:
        ...
        # This would ideally be based on how well the system adapts to student needs
        # For now, use a simplified calculation based on cognitive state
        engagement_level = cognitive_state.get("engagement_level", "medium")
        cognitive_load = cognitive_state.get("cognitive_load", "optimal")
        
        # Higher score for optimal engagement and cognitive load
        if engagement_level == "high" and cognitive_load == "optimal":
            return 0.9
        elif engagement_level == "medium" and cognitive_load == "optimal":
            return 0.7
        else:
            return 0.5
    ...
    def _calculate_knowledge_integration(self, state: ArchMentorState, analysis_result: Dict) -> float:
        ...
        # Check if student is connecting concepts and applying knowledge
        user_messages = [msg['content'] for msg in state.messages if msg.get('role') == 'user']
        ...
        # Look for evidence of knowledge integration
        integration_patterns = [
            "this connects to", "this relates to", "similar to", "like when",
            "because", "therefore", "this means", "so that"
        ]
        
        for msg in user_messages:
            if any(pattern in msg.lower() for pattern in integration_patterns):
                integration_indicators += 1
        
        integration_score = integration_indicators / total_messages
        return min(1.0, integration_score * 2)  # Scale up the score
    ...
    def enhance_response_with_scientific_context(self, challenge_result: Dict, scientific_metrics: Dict,
        ...
        """Enhance the challenge response with scientific context (metrics hidden from user)"""
        
        base_response = challenge_result.get("response_text", "")
        
        # Return only the base challenge - metrics are kept internal for analysis
        # The detailed scientific context is stored in cognitive_summary for internal use

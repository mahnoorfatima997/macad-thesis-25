"""
MEGA Architectural Mentor - Minimal Test Dashboard
Works without multi-agent dependencies
Supports Generic AI and Control groups only
"""
...
from thesis_tests.linkography_logger_simple import SimpleLinkographyLogger

# Page configuration
st.set_page_config(
    page_title="MEGA Test Dashboard (Minimal)",
    page_icon="🏗️",
    layout="wide",
    initial_sidebar_state="expanded"
)

class MinimalTestDashboard:
    """Minimal test dashboard for Generic AI and Control groups"""
    ...
    def render_sidebar(self):
        ...
        with st.sidebar:
            ...
            # Warning about minimal version
            st.warning("Running minimal version - MENTOR group not available")
            ...
            if not st.session_state.participant:
                ...
                test_group = st.selectbox(
                    "Test Group", 
                    [TestGroup.GENERIC_AI.value, TestGroup.CONTROL.value],
    ...
    def start_test_session(self, participant_id: str, test_group: TestGroup):
        ...
        st.session_state.linkography_logger = SimpleLinkographyLogger(
            session_id=st.session_state.test_session.id
        )
    ...
    def render_welcome_screen(self):
        ...
        st.title("MEGA Architectural Mentor - Test Dashboard (Minimal)")
        st.markdown("""
        ## Welcome to the Architectural Design Test
        
        This minimal version supports two test conditions:
        - **Generic AI Group**: Direct AI assistance (ChatGPT-like)
        - **Control Group**: No AI assistance
        
        Note: The MENTOR group requires additional dependencies not currently available.
    ...
    def render_test_environment(self):
        ...
        if st.session_state.test_group == TestGroup.GENERIC_AI:
    ...
    def export_session_data(self):
        ...
        if st.session_state.session_logger:
            ...
            st.success(f"Data exported to: {session_file}")
...
def main():
    """Main entry point"""
    dashboard = MinimalTestDashboard()
    dashboard.run()
